...
// Uniform directions let MongoDB use these indexes for reverse scans too.
ambassadorSchema.index({ firstname: 1, lastname: 1, _id: 1 });
ambassadorSchema.index({ 'activity.last_check_in': 1, _id: 1 });
...
ambassadorSchema.statics.markMigrationSmsSent = async function(ambassadorId) {
    try {
        ...
        logger.error({ err: error, ambassadorId }, 'Failed to mark migration SMS sent for ambassador');
    }
};
...
ambassadorSchema.statics.incrementTaskRequestedCount = async function(ambassadorId) {
    try {
        ...
        logger.error({ err: error, ambassadorId }, 'Failed to increment tasks_requested count for ambassador');
        // Rethrow so service-level catches stay live; swallowing here made them dead code.
        throw error;
    }
};
ambassadorSchema.statics.incrementTaskRequestedViewedCount = async function(ambassadorId) {
    try {
        ...
        logger.error({ err: error, ambassadorId }, 'Failed to increment tasks_requested_viewed count for ambassador');
        throw error;
    }
};
ambassadorSchema.statics.incrementTaskAcceptedCount = async function(ambassadorId) {
    try {
        ...
        logger.error({ err: error, ambassadorId }, 'Failed to increment tasks_requested_accepted count for ambassador');
        throw error;
    }
};
