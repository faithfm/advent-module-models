...
const taskSchema = new mongoose.Schema({
   ...
   //delivery|call|sms|email|social|bible|pastoral
    ...
    //requested->assigned->processed->sent->(ready)->completed

    //CALL/EMAIL/SMS/SOCIAL STATUS - PROCESS
    //requested->assigned->ready->completed
    ...
    status: {
      type: String,
      enum: ['new', 'requested', 'assigned', 'processed', 'sent', 'ready', 'completed'],
      default: 'new'      
    },
...
taskSchema.virtual('url_status_processed_qr').get(function() {
  return 'https://chart.googleapis.com/chart?cht=qr&chs=100x100&chl=' + querystring.escape(this.url_status_processed);
});
...
taskSchema.virtual('url_status_sent_qr').get(function() {
  return 'https://chart.googleapis.com/chart?cht=qr&chs=100x100&chl=' + querystring.escape(this.url_status_sent);
});
...
taskSchema.virtual('url_status_complete_qr').get(function() {
  return 'https://chart.googleapis.com/chart?cht=qr&chs=100x100&chl=' + querystring.escape(this.url_status_complete);
});
